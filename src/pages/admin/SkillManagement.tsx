...
import { Plus, Edit, Trash2, Save, X, Wrench } from 'lucide-react';
...
export default function SkillManagement() {
  ...
  const deleteSkill = async () => {
    ...
    try {
      setSaving(true);
      const { error } = await supabase.from('skills').delete().eq('id', toDelete.id);
      if (error) throw error;
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage the platform-wide skills designers can choose</p>
        </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={deleteSkill} disabled={saving}>{saving ? 'Deleting...' : 'Delete'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
