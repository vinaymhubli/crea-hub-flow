-- Insert refund policy sections data (only if not exists)
-- This migration only inserts data, assuming the table already exists

INSERT INTO public.website_sections (page, section_name, section_type, title, content, icon, background_color, is_published, sort_order) 
SELECT * FROM (VALUES
...
) AS v(page, section_name, section_type, title, content, icon, background_color, is_published, sort_order)
WHERE NOT EXISTS (
  SELECT 1 FROM public.website_sections 
  WHERE page = v.page AND section_name = v.section_name
);
